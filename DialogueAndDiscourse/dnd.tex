...
\title{Linguistic Alignment as Temporal-Causal Factor for  Sentiment Change}
\shorttitle{Does alignment cause sentiment change?}

% to do - talk to co-authors
%, Ngot Bui, and Vasant Honavar \\

\author{David Reitter, Yafei Wang\footnote{This work was done while the author was studying in Penn State}, John Yen, Ngot Bui, Vasant Honavar}
\affiliation{
...
Reciprocal linguistic adaptation between speakers is often seen as a contributor to mutual understanding. Parallelism is also a common rhetorical device.  However, whether adaptation can cause an emotional reaction of social consequence is less understood. 
...
\end{equation}.
\begin{equation}
\epsilon _{x} (c,e) = p(e|c\wedge x)-p(\neg e|c\wedge x)
\end{equation}
\end{defn}
...
Figure~\ref{fig:PCTL_Lex} and~\ref{fig:PCTL_Syn} show two probabilistic Kripke Structures constructed using lexical and syntactic alignment measures, respectively. The resulting Kripke structures show that $+A_T\wedge+r$ has the highest in-degree probability (e.g., $p(Ha\wedge+r,+o)$ is equal to $0.516$ and $0.455$ for lexical and syntactic probabilistic Kripke structures, respectively) among all  probabilities of transition into states of replies ($+A_T\wedge+r,La\wedge+r,Ha\wedge-r,La\wedge-r$). Thus, conversations in OHCs seem to favor social support with linguistic adaptation. 

\begin{figure}[!htb]
 \centering
  \includegraphics[width=0.99\linewidth]{Figures/model_lex_updated.pdf}
...
\begin{figure}[!htb]
 \centering
  \includegraphics[width=0.99\linewidth]{Figures/model_syn_updated.pdf}
...
Suppose we want to examine whether the positive sentiment together with high lexical alignment of replies, $+A_T \wedge +r$, is a prima facie cause of the positive sentiment of the latest post from thread initiator, $+f$, in a thread. With a cause $c=Ha \wedge +r$ and an effect $e=+f$, $c$ is a prima facie cause of $e$ if the probability of $c$ is larger than zero and the probability of $e$ conditional on $c$ is larger than the marginal probability of $e$ given the probabilistic Kripke structure in Figure~\ref{fig:PCTL_Lex}. Calculated using \textcite{hansson1994logic}, $+A_T\wedge+r$ is the prima cause of $+f$, since $+A_T\wedge+r\leadsto^{\geq 1,\leq \infty}+f = 0.618 $ and $b\leadsto^{\geq 1,\leq \infty}+f = 0.586$, where $b$ is the initial state in Figure~\ref{fig:PCTL_Lex} and~\ref{fig:PCTL_Syn}. Similarly, we calculate $c\leadsto^{\geq 1,\leq \infty}+f$ where $c \in \left \{+o, -o, Ha\wedge+r, Ha\wedge-r, La\wedge+r, La\wedge-r, +s, -s \right \}$. Then, $\left \{ +o, Ha \wedge +r \right \}$ and $\left \{ -o, Ha \wedge -r, La \wedge -r, -s \right \}$ are prima facie causes of $+f$ and $-f$, respectively, in both lexical and syntactic probabilistic Kripke structures. Our results parallel with the results in \textcite{bui2015temporal} that states with $+r$ and $-r$ are prima facie causes of $+f$ and $-f$, respectively. Also, these results parallel the Interactive Alignment Model \parencite{pickering2004toward} that linguistic alignment helps people improve the performance of the dialogue. 
...
{\bf to do: explain concept of causal significance in this framework.  also revise caption for associated table.}
...
First, $\epsilon(Ha \wedge +r, +f)$ appears in both lexical and syntactic alignment models. In other words, sentiment-positive replies that are either high lexical aligned or high syntactic aligned, play a key role in changing the sentiment of the latest post from the thread initiator in a thread. In contrast, sentiment-positive replies that are either low lexical aligned or low syntactic aligned, are not causal significant in \citegen{kleinberg_uai09} framework. Hence, high alignment at both lexical and syntactic level is a meaningful predictor for the final positive sentiment change.

In addition, both $\epsilon(Ha \wedge -r, -f)$ and $\epsilon(La \wedge -r, -f)$ are causal significant in both lexical and syntactic alignment models. Specifically, $\epsilon(Ha \wedge -r, -f)$ is smaller than $\epsilon(La \wedge -r, -f)$. It suggests that replies with negative sentiment have a stronger causal significant effect in combination with low alignment score than with high alignment score, at both lexical and syntactic levels in  \textcite{kleinberg_uai09} 's framework. Therefore, we conclude that the linguistic alignment of replies at both lexical and syntactic levels, together with the sentiment of the replies, causally affect the final sentiment change of the thread initiator.
...
$\epsilon(Ha \wedge +r, +f)$ & 0.033   & 0.030     \\ \hline
$\epsilon(Ha \wedge -r, -f)$ & 0.028   & 0.020     \\ \hline
$\epsilon(La \wedge -r, -f)$ & 0.046   & 0.051     \\ \hline
\end{tabular}
\caption{Level of Causal Significance of Prima Facie Causes about Alignment}
\label{tab:CausalSig}
\end{table}
...
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Figures/LexAposi_new_rob_New_Enlarge_updated.pdf}
  \caption{\label{fig:ProbLexPoso}}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Figures/LexAnegi_new_rob_New_Enlarge_updated.pdf}
...
\begin{subfigure}{.5\textwidth} 
  \centering
  \includegraphics[width=\linewidth]{Figures/SynAposi_new_rob_New_Enlarge_updated.pdf}
  \caption{\label{fig:ProbSynPoso}}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Figures/SynAnegi_new_rob_New_Enlarge_updated.pdf}
...
Figure~\ref{fig:posf05Lex} shows that $+A_T \wedge +r$ using the lexical alignment score consistently shows up as a prima facie cause. As the threshold of lexical alignment increases, the causal significance of $+A_T \wedge +r$ drops steadily. However, $-A_T \wedge +r$ is not considered as a prima facie cause (i.e. the causal significance of $-A_T \wedge +r$ is zero). This result parallels with the result in the previous section. Therefore, we conclude that $+A_T \wedge +r$ using the lexical alignment score is a genuine cause of $+f$, but $-A_T \wedge +r$ is not. This finding indicates that having replies with high \emph{lexical alignment} is an important factor on sentiment dynamics of the thread initiator in this OHC. 

Figure~\ref{fig:negf05Lex} presents that both $+A_T \wedge -r$ and $-A_T \wedge -r$ with the lexical alignment score are prima facie causes of $-f$. However, the causal significance of $-A_T \wedge -r$ is consistently and significantly (Wilcoxon signed-rank test, $p-value = 0.0004$) higher than that of $+A_T \wedge -r$ on $-f$. This result indicates that messages with negative sentiment complemented by lower lexical alignment likely cause the negative sentiment of thread initiator's final message in a thread. This finding parallels our causal analysis on $+f$ that messages with higher lexical alignment complemented with positive sentiment contribute on thread initiators' positive sentiment at the end of a thread. Further, the effect of $-A_T \wedge -r$ on $-f$ is larger than that of $+A_T \wedge -r$ on $-f$.
...
Further, Figure~\ref{fig:posf05Syn} shows that $+A_T \wedge +r$ using the syntactic alignment score consistently shows up as a prima facie cause. The causal significance of $+A_T \wedge +r$ drops substantially after increasing the threshold a little bit. However, $-A_T \wedge +r$ is considered as a prima facie cause after increasing the threshold a little bit (i.e. the causal significance of $-A_T \wedge +r$ is not zero). Therefore, we conclude that $+A_T \wedge +r$ likely causes $+f$. This finding indicates that having replies with high \emph{syntactic alignment} is not a strong factor on sentiment dynamics of the thread initiator in this OHC.

Figure~\ref{fig:negf05Lex} presents that both $+A_T \wedge -r$ and $-A_T \wedge -r$ with the syntactic alignment score are prima facie causes of $-f$. However, the causal significance of $-A_T \wedge -r$ is consistently and significantly (Wilcoxon signed-rank test, $p-value = 0.0001$) higher than that of $+A_T \wedge -r$ on $-f$. This result indicates that messages with negative sentiment complemented by lower syntactic alignment likely cause a more negative sentiment found in the thread initiator's final message. This finding complements the causal analysis on $+f$, which suggests that messages with higher syntactic alignment paired with positive sentiment contribute on thread initiators' positive sentiment at the end of a thread. Further, the effect of $-A_T \wedge -r$ on $-f$ is larger than that of $+A_T \wedge -r$ on $-f$.
...
In summary, messages with positive sentiment paired with high \emph{lexical} alignment is a genuine cause of the positive sentiment of thread initiator's final message. However, messages with positive sentiment paired with high \emph{syntactic} alignment represent a weaker cause.
...
The method used to come to this conclusion is  an example of how the temporal-causal inference framework can be applied in dialogue.
...
%The data are also compatible with a model of a language comprehension system where syntactic and lexical choices can make contributions to high-level meaning that are independent from each other.  
%Alignment has functions beyond a cascading system of building mutual understanding \parencite{pickering2004toward}.  
